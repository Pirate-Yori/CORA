  ...
  <div class="min-h-screen bg-gray-50">
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
          ...
          <div>
            ...
            <p class="text-gray-600">Explorez vos cours et suivez votre progression</p>
          </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mt-8">
          ...
          <div class="bg-green-50 rounded-xl p-4">
            <div class="text-2xl font-bold text-green-600 mb-1">{{ stats.enCours }}</div>
            <div class="text-sm text-green-700">En cours</div>
          </div>
          <div class="bg-purple-50 rounded-xl p-4">
            <div class="text-2xl font-bold text-purple-600 mb-1">{{ stats.terminees }}</div>
            <div class="text-sm text-purple-700">Terminées</div>
          </div>
          <div class="bg-orange-50 rounded-xl p-4">
            <div class="text-2xl font-bold text-orange-600 mb-1">{{ stats.progressionMoyenne }}%</div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        >
          ...
          <div :class="['relative h-32', matiere.colorBg]">
            ...
            <div class="absolute top-4 right-4">
              <span :class="['px-3 py-1 rounded-full text-xs font-semibold', matiere.statusColorBadge]">
                {{ matiere.statusLabel }}
              </span>
            </div>
            ...
            <div class="absolute inset-0 flex items-center justify-center">
              <div class="w-16 h-16 bg-white/20 backdrop-blur-sm rounded-2xl flex items-center justify-center text-4xl group-hover:scale-110 transition-transform">
                {{ matiere.icon }}
              </div>
            </div>
          </div>
          ...
          <div class="p-6">
            ...
            <div class="grid grid-cols-3 gap-3 mb-4">
              <div class="text-center">
                <div class="text-lg font-bold text-gray-800">{{ matiere.totalCours }}</div>
                <div class="text-xs text-gray-500">Cours</div>
              </div>
              <div class="text-center">
                <div class="text-lg font-bold text-gray-800">{{ matiere.dureeTotal }}</div>
                <div class="text-xs text-gray-500">Heures</div>
              </div>
              <div class="text-center">
                <div class="text-lg font-bold text-gray-800">{{ matiere.exercices }}</div>
            ...
            <div>
              <div class="flex items-center justify-between mb-2">
                ...
                <span class="text-xs font-semibold text-gray-700">{{ matiere.progression }}%</span>
              </div>
            ...
            <!-- Prochain cours -->
            <div v-if="matiere.prochainCours" class="mt-4 pt-4 border-t border-gray-100">
              <div class="flex items-center gap-2 text-sm text-gray-600">
                <svg class="w-4 h-4 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
                <span class="text-xs">Prochain cours : <span class="font-medium">{{ matiere.prochainCours }}</span></span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div v-else class="space-y-4">
        ...
        >
          <div class="flex flex-col sm:flex-row items-start sm:items-center gap-6 p-6">
            <!-- Icône et info -->
            <div class="flex items-center gap-4 flex-1">
              <div :class="['w-16 h-16 rounded-2xl flex items-center justify-center text-3xl', matiere.colorBg]">
                {{ matiere.icon }}
              </div>
            ...
            <div class="w-full sm:w-48">
              <div class="flex items-center justify-between mb-2">
                <span class="text-xs text-gray-500">Progression</span>
                <span class="text-sm font-semibold text-gray-700">{{ matiere.progression }}%</span>
              </div>
              <div class="w-full bg-gray-200 rounded-full h-2">
                <div 
                  :class="['h-2 rounded-full transition-all', matiere.progressColor]"
      ...
      <div v-if="matieresFiltered.length === 0" class="text-center py-16">
        ...
        <h3 class="text-lg font-semibold text-gray-800 mb-2">Aucune matière trouvée</h3>
...
const matieresFiltered = computed(() => {
  ...
  if (searchQuery.value) {
    filtered = filtered.filter(m => 
      ...
      m.professeur.toLowerCase().includes(searchQuery.value.toLowerCase())
    );
  }

  // Filtre par statut
  if (filtreStatut.value !== 'tous') {
    filtered = filtered.filter(m => m.statut === filtreStatut.value);
  }
